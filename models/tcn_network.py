...
from tensorflow.keras.layers import Input, Activation, Dense,Flatten, BatchNormalization, Lambda,add,GlobalAveragePooling2D
from tensorflow.keras.layers import SpatialDropout2D,Add, Conv2D, MaxPooling2D,AveragePooling2D,Layer,ZeroPadding2D,Conv1D
import numpy as np
...
""" Code borrowed from https://github.com/philipperemy/keras-tcn"""
...
class ResidualBlock_TCN(Layer):
    ...
    def __init__(self,
        ...
        super(ResidualBlock_TCN, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        with K.name_scope(self.name):  # name scope used to make sure weights get unique names
            ...
            for k in range(2):
                ...
                with K.name_scope(name):  # name scope used to make sure weights get unique names
                    if self.padding == 'causal':
                        self._add_and_activate_layer(ZeroPadding2D(padding=((0,0),(self.dilation_rate * (self.kernel_size-1),0))))
                        self._add_and_activate_layer(Conv2D(filters=self.nb_filters,
                ...
                with K.name_scope('norm_{}'.format(k)):
                    ...
                    self._add_and_activate_layer(normalize_layer(self.type_norm))
            ...
            super(ResidualBlock_TCN, self).build(input_shape)  # done to make sure self.built is set True
...
class TCN(Layer):
    ...
    def build(self, input_shape):
        ...
        for s in range(self.nb_stacks):
            for i, d in enumerate(self.dilations):
                ...
                self.residual_blocks.append(ResidualBlock_TCN(dilation_rate=d,
        ...
        if self.padding == 'same':
            time = self.build_output_shape.as_list()[2]
            if time is not None:  # if time dimension is defined. e.g. shape = (bs, 500, input_dim).
                self.output_slice_index = int(self.build_output_shape.as_list()[2] / 2)
            else:
    ...
    def get_config(self):
        ...
        config['type_norm'] = self.type_norm
        config['kernel_initializer'] = self.kernel_initializer
        return config

def compiled_tcn(input_shape,  # type: tuple(Ticker,Timestep,feat)
                 nb_filters,  # type: int
    ...
    Args:
        input_shape: shape of your input, i.e. the last dimension of: (N_ticker, timesteps, input_dim).
        ...
        kernel_initializer: Initializer for the kernel weights matrix (Conv2D).
        type_norm: type of norm layer
    Returns:
        A TCN model.
    """
    ...
    x = TCN(nb_filters, kernel_size, nb_stacks, dilations, padding,
            ...
            activation, kernel_initializer, type_norm,
    ...
    # regression
    ### Correlation of tickers
    # x = K.print_tensor(x)
    ## command conv2d
    out_shape = K.int_shape(x)
    # x = Conv2D(filters=nb_filters,
    #             kernel_size=(input_shape[0],out_shape[2]),
    #             padding='valid',
    #             name='Corr_conv2D',
    #             kernel_initializer=kernel_initializer)(x)
    
    # x = normalize_layer(type_norm)(x)

    # # out_shape = K.shape(x)
    
    # x = GlobalAveragePooling2D()(x)
    x = Conv1D(nb_filters, kernel_size=input_shape[0], padding='valid',
                name ='Corr_conv1D',
                kernel_initializer=kernel_initializer) (x)
    ...
    ## Return model have not compiled yet
    return model